...
def uninstall_rke2(node, is_server=True):
    ...
    try:
        ...
        commands = [
            f"echo {node['password']} | sudo -S -S systemctl stop rke2-{service_type}",
            f"echo {node['password']} | sudo -S -S systemctl disable rke2-{service_type}"
        ]
        ...
        for script_path in uninstall_paths:
            ...
            if stdout.read().decode().strip() == 'exists':
                stdin, stdout, stderr = ssh.exec_command(f"echo {node['password']} | sudo -S -S {script_path}")
                exit_code = stdout.channel.recv_exit_status()
                if exit_code == 0:
        ...
        cleanup_commands = [
            f"echo {node['password']} | sudo -S rm -rf /var/lib/rancher/rke2",
            f"echo {node['password']} | sudo -S rm -rf /etc/rancher/rke2",
            f"echo {node['password']} | sudo -S rm -rf /var/lib/kubelet",
            f"echo {node['password']} | sudo -S rm -rf /opt/rke2",
            f"echo {node['password']} | sudo -S rm -f /usr/local/bin/rke2",
            f"echo {node['password']} | sudo -S rm -f /usr/local/bin/kubectl",
            f"echo {node['password']} | sudo -S -S rm -f /usr/bin/rke2",
            f"echo {node['password']} | sudo -S rm -f /etc/systemd/system/rke2-*.service",
            f"echo {node['password']} | sudo -S rm -f /usr/share/rke2"
        ]
        ...
        network_commands = [
            f"echo {node['password']} | sudo -S ip link delete flannel.1 2>/dev/null || true",
            f"echo {node['password']} | sudo -S ip link delete cni0 2>/dev/null || true",
            f"echo {node['password']} | sudo -S ip link delete vxlan.calico 2>/dev/null || true"
        ]
        ...
        if is_server:
            ...
            firewall_commands = [
                f"echo {node['password']} | sudo -S firewall-cmd --permanent --remove-port=9345/tcp || true",
                f"echo {node['password']} | sudo -S firewall-cmd --permanent --remove-port=6443/tcp || true", 
                f"echo {node['password']} | sudo -S firewall-cmd --permanent --remove-port=8472/udp || true",
                f"echo {node['password']} | sudo -S firewall-cmd --permanent --remove-port=10250/tcp || true",
                f"echo {node['password']} | sudo -S firewall-cmd --reload || true"
            ]
